...
public class DBUtils {
	...
	public static ArrayList<Lesson> getLesson(Connection conn, String Email)
		...
		while (rs.next()) {
			...
			String namesub = rs.getString("namesub");

			Lesson lesson = new Lesson(idvideo, linkimg, title, email, namesub);
			lst.add(lesson);
		}
		return lst;
	}
	...
	public static Boolean insertLesson(Connection conn, Lesson lesson)
			throws SQLException {
		String sql = "Insert into lesson(idvideo, emailuser, title, linkimg, namesub) values (?, ?, ?, ?, ?)";
		...
		pstm.setString(5, lesson.getNameSub());
